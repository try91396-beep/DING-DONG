...
def check_new_orders():
    ...
    if current_max > 0 and max_seq_val > current_max:
        ...
        if new_order_ids:
            ...
            print(f"[{now}] 🔔 偵測到新訂單，準備觸發列印: {', '.join(seq_list)}")
...
# --- 3. 補印功能 (Android RawBT + Windows 雙模組整合版) ---
...
def print_order(oid):
    try:
        ...
        def generate_html(title, item_list, is_receipt=False):
            ...
            if is_receipt: 
                h += f"<div class='total'>${int(total_price or 0)}</div>"
        ...
        if print_type in ['all', 'receipt']:
            content += generate_html("結帳單", items, is_receipt=True)
            has_content = True if items else has_content
        ...
        # --- 雙模組核心邏輯 (Windows / Android 自動切換) ---
        
        # 1. 準備給 Android RawBT 用的 Base64 資料
        ...
        # RawBT Intent 格式 (包含 S.editor=false 設定)
        intent_url = (
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {{
                ...
                if (/android/i.test(userAgent)) {{
                    // --- Android 模式：呼叫 RawBT ---
                    document.body.innerHTML = '<h2 style="text-align:center; padding-top:50px; color:green;">🖨️ Android: 正在傳送至出單機...</h2>';
                    ...
                    // --- Windows / PC / iOS 模式：呼叫瀏覽器列印 ---
                    window.print();
                    
                    // 注意：Windows 上通常不自動關閉，以免中斷預覽視窗
                    // 若您希望 Windows 印完嘗試關閉，可取消下方註解：
                    /*
                    setTimeout(function() {{
                         if(window.opener) window.close();
                    }}, 1000); 
        ...
        # 3. 回傳最終頁面 (包含收據內容 + 判斷腳本)
        final_render_html = f"<html><head>{style}</head><body>{content}{script_logic}</body></html>"
