...
import json
import base64  # <--- ⚠️ 絕對不能刪除這行，否則會報錯
import traceback 
...
# --- 3. 補印功能 (Android RawBT + Windows 雙模組) ---
...
def print_order(oid):
    try:
        ...
        # RawBT Intent (加入 S.editor=false)
        intent_url = (
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {{
                ...
                if (/android/i.test(userAgent)) {{
                    ...
                    // --- Windows / PC / iOS: 呼叫瀏覽器列印 ---
                    window.print();
                    
                    // Windows 這裡通常不建議自動關閉，因為會中斷預覽視窗
                    // 如果您確定要自動關閉，可以取消下方註解：
                    /*
                    setTimeout(function() {{
                        if(window.opener) window.close();
        ...
        # 3. 回傳完整頁面 (包含內容 + 判斷腳本)
        # 這樣 Windows 才能印出 "content"，Android 才能執行 "script"
        final_html = f"<html><head>{style}</head><body>{content}{script_logic}</body></html>"
        return final_html
